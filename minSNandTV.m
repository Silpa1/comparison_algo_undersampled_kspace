...
for out = single(1:opts.outer_iter),
 ...
 [U,earray1] = CG_solver(b,A, At,D,Dt,Lambda,Lam1,Lam2,Lam3,Lam4,Wx,Wy,Wt,opts, U, 1e-7,30);
%% plot a frame of the recon while it iterates
   %figure(3); 
   %imagesc(abs(double((U(:,:,31))))); title('A frame of the reconstruction'); colormap(gray);
